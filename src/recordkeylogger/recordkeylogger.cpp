                               ...
                               QByteArray &chars, int &count)
{
    ...
    // translate X modifiers to Qt::KeyboardModifiers
    ...
    if (xmodifiers & AltMask) {
        modifiers |= Qt::AltModifier;
    }

    if (xmodifiers & MetaMask) {
...
static void initModifiers(Display *dpy)
{
...
static void createXKeyEvent(XRecordDatum *data, XKeyEvent *event, Display *display)
{

    event->keycode = data->event.u.u.detail;
    event->type = data->type;
    event->state = data->event.u.keyButtonPointer.state;
    event->display = display;
    event->root = data->event.u.keyButtonPointer.root;
    event->same_screen = data->event.u.keyButtonPointer.sameScreen;
    event->x = data->event.u.keyButtonPointer.eventX;
    event->y = data->event.u.keyButtonPointer.eventY;
    event->x_root = data->event.u.keyButtonPointer.rootX;
    event->y_root = data->event.u.keyButtonPointer.rootY;
    event->time = data->event.u.keyButtonPointer.time;

}


static void processKeyEvent(XRecordDatum *data, WatchData *watch, KeyloggerEvent *event)
{

    XKeyEvent xevent;
    createXKeyEvent(data, &xevent, watch->ctrl_display);
    ...
    int count = XLookupString(&xevent, chars.data(), chars.size(), &sym, 0);
    const QString text = translateKeySym(sym, data->event.u.keyButtonPointer.state, code, modifiers, chars, count);

    event->setText(text);
    event->setModifiers(modifiers);
    event->setId(data->event.u.u.detail);
    event->setKey((Qt::Key) code);
    event->setCount(qMax(qMax(count, 1), text.length()));
...
static void event_callback(XPointer priv, XRecordInterceptData *hook)
{
    ...
    const int buttonCode = data->event.u.u.detail;
    ...
    case KeyPress: {
            ...
            processKeyEvent(data, watch, &event);
            break;
        }
    case KeyRelease: {
            event.setType(RecordItNow::KeyloggerEvent::KeyboardEvent);
...
bool RecordKeylogger::start(const KConfig *)
{
    ...
    initModifiers(d->data->data_display);
